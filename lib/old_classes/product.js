...
module.exports = class Product
{
    ...
    async load( scope )
    {
        ...
        if( this.#data.group?.[country+'_'+locale] === undefined )
        {
            promises.push( this.#ctx.core.call( 'get_product', { id: this.id, scope: 'group' }, { country, locale }).then( async( product ) =>
            {
                ObjectMerge( this.#data,
                {
                    group: 
                    {
                        [country+'_'+locale]: product.group // TODO locale & country
                    }
                },
                {
                    group:
                    {
                        [country+'_'+locale]: { url: '/' + product.group.name.slugify() + '-g' + product.group.id }
                    }
                });
            }));
        }
    ...
    get group()         { return this.#data.group?.[ this._scope().country+'_'+this._scope().locale ]}
    ...
    async trend( score = 1 )
    {
        //console.log( 'product/' + this.id + '/trend', score );
        return this.#ctx.client.get( 'product/' + this.id + '/trend', { query: { ...this.#ctx.scope(), score }}).then( r => r.json );
    }
