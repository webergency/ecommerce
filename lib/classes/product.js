...
module.exports = class Product
{
    ...
    async load( scope = 'list' )
    {
        ...
        {
            promises.push( this.#ctx.client.get( 'product', { query: { id: this.id, country, locale, scope }}).then( r => r.json ).then( async( product ) =>
            {
                let parameters = [];
        ...
        {
            promises.push( this.#ctx.client.get( 'product', { query: { id: this.id, country, locale, scope: 'group' }}).then( r => r.json ).then( async( product ) =>
            {
                ObjectMerge( this.#data, 
                {
                    group: 
        ...
        if( this.#data.description?.[locale] === undefined || this.#data.availability?.[country] === undefined )
        {
            promises.push( this.#ctx.client.get( 'product', { query: { id: this.id, country, locale, scope: 'detail' }}).then( r => r.json ).then( async( product ) =>
            {
                product.categories = await Promise.all( product.categories.map( id => ( require('./category') ).get( this.#ctx, id ))); // TODO preload pre locale
