...
module.exports = class Product
{
    ...
    async load( scope = 'group' )
    {
        ...
        if( [ 'group', 'detail' ].includes( scope ) && this.#data.group === undefined )
        {
            promises.push( this.#ctx.core.call( 'get_product', { id: this.id, scope: 'group' }, { country, locale }).then( async( product ) =>
            {
                product.group.name = product.group.name || 'skupina ' + this.id;
    ...
    async similar( count )
    {
        ...
        .then( r => r.json ).then( s => s.map( id => Product.get( this.#ctx, id, 'group' )));
    }
}