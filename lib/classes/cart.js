...
class CartProducts extends Array
{
    ...
    async set( productID, quantity, data ) // TODO zlepsit data - vyrobit dalsi produkt ked sa lisia v data
    {
        ...
        else
        {
            ...
            if( entry.quantity > 0 )
            {
                console.log( entry.product.availability );
...
module.exports = class Cart
{
    ...
    constructor( ctx, id )
    {
        ...
        //Object.defineProperty( this, 'products', { value: new CartProducts({ ...this.#ctx, cart: this }), writable: false, enumerable: true });
        ...
        Object.defineProperty( this.#customer, 'set', { value: async( data ) =>
        {
            ...
            if( data.hasOwnProperty( 'vat' ) && this.#customer.vat !== data.vat )
            {
                ...
                if( data.vat )
                {
                    try
                    {
                        let validation = await Validator.company({ vat: data.vat = Validator.normalize( data.vat, 'vat' )});

                        this.#noVAT = ( validation.length && validation[0].vat === data.vat && validation[0].vat.substr(2) !== 'SK' ); // TODO dat krajinu z configu
                    }
                    catch(e)
                    {
                        console.log( e );

                        validation.vat = false;
                    }
                }
