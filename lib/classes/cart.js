    ...
    async set( productID, quantity, data ) // TODO zlepsit data - vyrobit dalsi produkt ked sa lisia v data
    {
        ...
        else
        {
            ...
            if( typeof quantity === 'string' && QUANTITY_RE.test( quantity ))
            {
                ...
                {
                    this.unshift( entry = { product: await Product.get( this.#ctx, product_id, 'list' ), quantity, data });
                }
            }
            else
            {
                entry ? entry.quantity = parseFloat( quantity ) : this.push( entry = { product: await Product.get( this.#ctx, product_id, 'list' ), quantity: parseFloat( quantity ), data });
            }
