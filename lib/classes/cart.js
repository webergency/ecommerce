...
class CartProducts extends Array
{
    ...
    async set( productID, quantity, data ) // TODO zlepsit data - vyrobit dalsi produkt ked sa lisia v data
    {
        ...
        else
        {
            ...
            let product = entry ? entry.product : await Product.get( this.#ctx, productID, 'list' );
            ...
            if( typeof quantity === 'string' && QUANTITY_RE.test( quantity ))
            {
                ...
                {
                    this.unshift( entry = { product, quantity, data });
                }
            }
            else
            {
                entry ? entry.quantity = parseFloat( quantity ) : this.push( entry = { product, quantity: parseFloat( quantity ), data });
            }
    ...
    async _sync()
    {
        return this.#cart.id && await this.#ctx.client.put( 'session/' + this.#cart.id + '/cart/products', { body: this.map( p => ({ productID: p.product.id, quantity: p.quantity, data: p.data }))}).then( r => r.text );
    }
}
...
module.exports = class Cart
{
        ...
        Object.defineProperty( this.#customer, 'set', { value: async( data ) =>
        {
            ...
            if( data.hasOwnProperty( 'vat' ) && this.#customer.vat !== data.vat )
            {
                if( data.vat )
                {
                    ...
                    this.#noVAT = ( companies.length && companies[0].vat === data.vat && companies[0].vat.substr(0, 2) !== 'SK' ); // TODO dat krajinu z configu
                }
    ...
    get payments()
    {
        ...
        payment: for( let payment of this.#ctx.company.payments )
        {
            ...
            payments.push(
            {
                ...
                price   :
                {
                    current     : toDecimals( payment.price[country] / ( this.#noVAT ? ( 1 + payment.vat[country] ) : 1 ), 2 ),
                    vat         : this.#noVAT ? 0 : payment.vat[country],
                    discount    : 0
                },
                data    : payment.data
            });
        ...
        for( let shipping of this.#ctx.company.shippings )
        {
            pkg: for( let pkg of shipping.packages )
            {
                ...
                shippings.push(
                {
                    ...
                    price       :
                    {
                        current     : toDecimals( pkg.price[country] / ( this.#noVAT ? ( 1 + pkg.vat[country] ) : 1 ), 2 ),
                        vat         : this.#noVAT ? 0 : pkg.vat[country],
                        discount    : 0
                    },
    ...
    async submit( price, options )
    {
        ...
        await Promise.all( this.products.map( p => p.product.load( 'detail' )));

        for( let product of this.products )
        {
            product.product.trend( 5000 );
        }
        ...
        for( let item of this.products )
        {
            ...
            {
                for( let _supplier of item.product.suppliers )
                {
                    if( _supplier.availability.stock && ( _supplier.id == 'maxifleur' || _supplier.id == 'eurogreens' ))
                    {
                        supplier =
                        {
                            id          : _supplier.id || '',
                            productID   : _supplier.productID || '',
                            variationID : _supplier.variationID || '',
                            UID         : _supplier.productID + ( _supplier.variationID ? ':' + _supplier.variationID : '' ),
                            url         : _supplier.data?.url ? _supplier.data?.url : ''
                        }
                    }
                }
            }
            else
            {
                supplier =
                {
                    ...
                    productID   : item.product.suppliers[0].productID || '',
                    variationID : item.product.suppliers[0].variationID || '',
                    UID         : item.product.suppliers[0].productID + ( item.product.suppliers[0].variationID ? ':' + item.product.suppliers[0].variationID : '' ),
                    url         : item.product.suppliers[0].data?.url ? item.product.suppliers[0].data?.url : ''
                }
            }
            ...
            products.push(
            {
                ...
                supplierVariationID : supplier.variationID,
                supplierUID         : supplier.UID,
                physical            : 1,
                data                :
                {
                    image: item.product.images.length ? item.product.images[0].url : null,
                    url: 'https://' + website.domain + item.product.url,
        ...
        order.estimatedTime = NOW_S() + 24 * 60 * 60 * ( estimatedTime + shipping.delivery );
    ...
    async _sync()
    {
        return this.id && await this.#ctx.client.put( 'session/' + this.id + '/cart/customer', { body: this.customer }).then( r => r.text );
    }
