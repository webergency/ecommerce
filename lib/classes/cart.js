    ...
    async set( product_id, quantity, data )
    {
        ...
        else
        {
            ...
            if( typeof quantity === 'string' && QUANTITY_RE.test( quantity ))
            {
                ...
                {
                    this.#list.unshift({ product: await Product.get( this.#ctx, product_id, 'list' ), quantity });
                }
            }
            else
            {
                entry ? entry.quantity = parseInt( quantity ) : this.#list.push({ product: await Product.get( this.#ctx, product_id, 'list' ), quantity: parseInt( quantity )});
            }
        }
