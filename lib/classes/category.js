...
const Category = module.exports = class Category
{
    ...
    get url()           { return '/' + this.name.slugify() + '-c' + this.id }
    ...
    async siblings()
    {
        ...
        if( !this.#data.siblings )
        {
            ...
            this.#data.siblings = await Promise.all( siblings.map( id => Category.get( this.#ctx, id, 'list' )));
        }
    ...
    async children()
    {
        ...
        if( !this.#data.children )
        {
            ...
            this.#data.children = await Promise.all( children.map( id => Category.get( this.#ctx, id, 'list' )));
        }
    ...
    async breadcrumb()
    {
        ...
        if( !this.#data.breadcrumb )
        {
            ...
            this.#data.breadcrumb = await Promise.all( breadcrumb.map( id => Category.get( this.#ctx, id, 'list' )));
        }
