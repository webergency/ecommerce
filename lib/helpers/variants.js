...
function get_products_distance( product1, product2, parameters )
{
    ...
    for( let parameter of parameters )
    {
        ...
        {
            score += Math.max( parameter.priority || 0, 0.1 );
            ++parameters_cnt;
        }
    }

    return { score, parameters: parameters_cnt };
}
...
module.exports = function variants( group, options = {})
{
    ...
    if( options.product )
    {
        for( let variation of variations )
        {
            ...
            {
                value.products.sort(( a, b ) => get_products_distance( options.product, a, distinct_parameters ).score - get_products_distance( options.product, b, distinct_parameters ).score );

                value.selected = value.products[0] === options.product;
                value.enabled = ( value.products[0] === options.product ) ? options.product.visible : get_products_distance( options.product, value.products[0], distinct_parameters ).parameters < 2;
            }
        }
    }
